  ...
  NioApp.Plugins.countdown = function () {
    ...
    if ($count.exists()) {
      $count.each(function () {
        ...
        $self.countdown(new Date(datetime)).on("update.countdown", function (event) {
          $(this).html(
            event.strftime(
...
(function ($, window, document) {
  $("document").ready(function () {
    ...
    $.get("https://api.app.letstudy.io/token-data/lsc").done(function (data) {
      if (!data) {
        return;
      }
      ...
      $("#pcsw_liquidity").text("$" + numberWithCommas(Math.round(summary.liquidityUSD || 0) + ""));
      $("#pcsw_price_usd").text("$" + Number(summary.priceUSD).toFixed(2));

      if (summary.priceUSDChange > 0) {
        $("#pcsw_price_usd_change").text("↑" + Number(summary.priceUSDChange).toFixed(2) + "%");
      }

      if (summary.liquidityUSDChange > 0) {
        $("#pcsw_liquidity_change").text("↑" + Number(summary.liquidityUSDChange).toFixed(2) + "%");
      }

      const volumeChartData = [];
      const formattedVolumeData = data.chart.formattedVolumeData || [];
      for (let i = 0; i < formattedVolumeData.length; i++) {
        let item = {};
        item["key"] = formattedVolumeData[i].time.split("-")[0];
        item["value"] = +formattedVolumeData[i].value;
        volumeChartData.push(item);
      }

      const priceLightweightCharts = LightweightCharts.createChart(document.querySelector("#pcsw_price_chart"), {
        width: $("#pcsw_price_chart").parent().width(),
        height: 300,
      });
      const candlestickSeries = priceLightweightCharts.addCandlestickSeries({
        upColor: "#31d0aa",
        downColor: "#ed4b9e",
        borderVisible: false,
      });
      ...
      const liquidityChartData = data.chart.formattedTvlData || [];

      const liquidityLightweightCharts = LightweightCharts.createChart(
        document.querySelector("#pcsw_liquidity_chart"),
        { width: $("#pcsw_liquidity_chart").parent().width(), height: 300 }
      );

      const areaSeries = liquidityLightweightCharts.addAreaSeries({
      ...
      areaSeries.setData(liquidityChartData);
    });
